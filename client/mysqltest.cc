                     ...
                     int column, DYNAMIC_STRING* ds)
{
  ...
  if (mysql_query(mysql, query))
  {
    report_or_die("'%s' failed: %d %s on connection %s", query,
                  ...
                  const char *db, int port, const char *sock)
{
  ...
  if (catalog)
  {
    ...
    {
      /*
        We only come here when using the server --version option in test
        and run mtr with --use-catalog=...
        In this case the C connector will not use the catalog connect option
        and we have to change catalog explicitely.
      */
      strxnmov(query, sizeof(query)-1, "USE CATALOG `", catalog, "`", NullS);
      if (mysql_query(mysql, query))
      {
        die("Could not change to catalog '%s', error: '%s'",
            catalog, mysql_error(mysql));
      }
    }
  }
#endif
...
void do_connection_exists(struct st_command *command)
{
  int error;
  static DYNAMIC_STRING ds_connection;
  const struct command_arg con_exist_args[] = {
    { "connection", ARG_STRING, TRUE, &ds_connection,
      "Connectionn to check if it exist" }
  };
  DBUG_ENTER("do_connection_exist");

  check_command_args(command, command->first_argument,
                     con_exist_args,
                     sizeof(con_exist_args)/sizeof(struct command_arg),
                     ' ');

  error= !find_connection_by_name(ds_connection.str);
  var_set_int("$error",error);
  dynstr_free(&ds_connection);
  DBUG_VOID_RETURN;
}
                  ...
                  const char *err_sqlstate, DYNAMIC_STRING *ds)
{
  ...
  if (command->abort_on_error)
  {
    report_or_die("query '%s' failed: %s (%d): '%s' on connection '%s'",
                  command->query,
                  get_errname_from_code(err_errno),
                  err_errno,
                  err_error,
  ...
  while (!abort_flag && !read_command(&command))
  {
    ...
    */
    ...
    if (ok_to_do)
    {
      ...
      case Q_CONNECTION_EXISTS: do_connection_exists(command); break;
      case Q_DISCONNECT:
      case Q_DIRTY_CLOSE:
