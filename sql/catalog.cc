...
bool check_catalog_access(THD *thd, const LEX_CSTRING *name)
{
  ...
  if (!(sctx->master_access & CATALOG_ACL))
  {
    ...
    my_error(ER_CATALOG_ACCESS_DENIED_ERROR, MYF(ME_ERROR_LOG),
             sctx->priv_user, sctx->host_or_ip, name->length,
             name->str);
    return true;
  }
  return false;
                        ...
                        const LEX_CSTRING *name)
{
  ...
  */
  ...
  memcpy(ptr, create.schema_comment, create.schema_comment->length+1);
  cat->comment.set(ptr, create.schema_comment->length, default_charset_info);
...
bool init_catalogs(const char *datadir)
{
  ...
  internal_default_catalog.comment.set(default_catalog_comment.str,
                             ...
                             Schema_specification_st *create_info)
{
  ...
  */
  ...
  if (create_info->schema_comment)
    catalog->comment.copy(create_info->schema_comment->str,
